<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="detail">
        <h5>商品清单</h5>
        <ul class="list clearFix" v-for="orderDetail in detailArrayList" :key="orderDetail.skuId">
          <li>
            <img :src="orderDetail.imgUrl" alt="" style="width: 100px">
          </li>
          <li>
            <p>
              {{ orderDetail.skuName }}
...
export default {
  ...
  methods: {
    // 选择默认收件人信息
    changeDefult(address, addressList) {
      // 排他
    ...
    // 提交订单
    async submitOrder() {
      let { tradeNo } = this.tradeInfo; //订单编号
      // 订单信息
      let data = {
        "consignee": this.userSelectAddress.consignee, // 收件人
        "consigneeTel": this.userSelectAddress.phoneNum,  // 电话
        "deliveryAddress": this.userSelectAddress.fullAddress, // 地址
        "paymentWay": "ONLINE", // 在线支付
        "orderComment": this.userMessage,  //  留言
        "orderDetailList": this.detailArrayList // 商品信息数组
      }
      try {
        let result = await this.$API.reqSubmitOrder(tradeNo, data); // 发送请求
        // 获取返回订单编号
        this.resultOrderId = result.data;
        //跳转支付：路由传参订单编号
        this.$router.push({
          name: 'pay',
          query: {
